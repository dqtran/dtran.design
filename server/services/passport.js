 ...
 * Passport Service
 *
 * A painless Passport.js service for your app that is guaranteed to
...
passport.connect = function (req, query, profile, next) {
  ...
  obj.provider = req.params.provider;

  // Use profile.provider or fallback to the query.provider if it is undefined
  // as is the case for OpenID, for example
  provider = profile.provider || obj.provider;
  ...
  var accessToken = obj.tokens.accessToken;
  ...
  abe.db.Passport.findOne(dbQuery)
    ...
    if (!req.me) {
      ...
      if (!passport) {
        ...
        .then(function (user) {
            ...
            abe.db.Passport.create(dbQuery)
            .then(function (passport) {
                next(null, user);
            })
            .catch(function(err){
      ...
      // Action:   Get the user associated with the passport.
        ...
        if (obj.hasOwnProperty('tokens') && obj.tokens !== passport.tokens) {
          passport.tokens = obj.tokens;
          passport.accessToken = obj.tokens.accessToken;
        }
...
passport.callback = function (req, res, next) {
  var provider = req.params.provider || 'local',
...
passport.loadStrategies = function () {
  var self       = this,
    strategies = abe.config.passport;
  ...
  Object.keys(strategies).forEach(function (key) {
    ...
    if (key === 'local') {
      ...
      var baseUrl = abe.config.env.baseURL;
...
passport.disconnect = function (req, res, next) {
  ...
  abe.db.Passport.findOne({
        ...
        abe.db.Passport.destroy(passport.id)
        .then(function (passport) {
            next(null, user);
        })
        .catch(next);
...
passport.deserializeUser(function (id, next) {
  ...
  abe.services.user.getById(id, next);

});


function creatToken(done){
